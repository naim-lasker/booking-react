...
import { editProviderMenu } from "../../../../services/menu"
...
const EditMenuModal = ({ show, onHide, menu }) => {
    ...
    const menuNameRef = createRef()

    const setMenuInfo = () => {
        setMenuId(menu && menu.id ? menu.id : 0)
        setMenuName(
            menu && menu.menu_name ? menu.menu_name : ""
        )
        setOverview(menu && menu.overview ? menu.overview : "")
        setAdditionalInfo(
            menu && menu.additional_info ? menu.additional_info : ""
        )
        setSellingPrice(
            menu && menu.selling_price ? menu.selling_price : 0
        )
        setVatCodePercentence(menu && menu.vat ? menu.vat : 0)
        setAvailabilityStatus(
            menu && menu.availability_status == 1 ? true : false
        )
        setAvailabilityFrom(
            menu && menu.availability_from
                ? menu.availability_from
                : ""
        )
        setAvailabilityTo(
            menu && menu.availability_to ? menu.availability_to : ""
        )
        setDiscountStatus(
            menu && menu.discount_status == 1 ? true : false
        )
        setDiscountPercentence(
            menu && menu.discount_percentage
    ...
    const checkSubmit = () => {
        if (!menuName) {
            ...
            notify("error", "Please enter menu name")
        } else {
    ...
    const handleSubmit = () => {
        ...
        dispatch(
            editProviderMenu(menuObj, menuId, (res, err) => {
                setLoading(false)
                setConfirmAlert(false)
                window.location.href = "/provider-menu-list"
    ...
    return (
        ...
        >
            <Modal.Header closeButton>
                <Modal.Title id='edit-menu'>Update Menu</Modal.Title>
            </Modal.Header>
            <Modal.Body className='mx-3'>
                ...
                {/* <div className='upload-container row justify-content-center align-items-center flex-column '>
                    ...
                    <h3 className='upload-img-header'>Upload menu image</h3>
                </div> */}
